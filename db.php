...
class db
{
	...
	public function where_or($wh_input, $in_value = false)
	{
		if (is_array($wh_input))
		{
			if (count($wh_input) > 0)
			{
				$wg_begin = (!empty($this->where_data)) ? ' AND (' : '';
				$this->where_data .= $wg_begin.implode(' ? OR ', array_keys($wh_input)).' ?)';
				foreach ($wh_input as $key => $value) $this->bind_data[] = (string)$value;
			}
		}
		else
		{
			if (!empty($wh_input))
			{
				$wg_begin = (!empty($this->where_data)) ? ' OR ' : '';
				$this->where_data .= $wg_begin.$wh_input.' ?';
				$this->bind_data[] = $in_value;
			}
		}
	...
	public function group_by($group_by)
	{
		if (!empty($group_by))
		{
			$group_begin = (!empty($this->group_data)) ? ', ' : '';
			$this->group_data .= $group_begin.$group_by;
		}
	...
	public function run($table)
	{
		...
		$group = (!empty($this->group_data)) ? ' GROUP BY '.$this->group_data : '';
		$limit = (is_numeric($this->limit_data) && intval($this->limit_data) > 0) ? ' LIMIT '.intval($this->limit_data) : '';

		$query = "SELECT ".$this->select_data." FROM ".$table.$this->join_data.$where.$group.$order.$limit;
